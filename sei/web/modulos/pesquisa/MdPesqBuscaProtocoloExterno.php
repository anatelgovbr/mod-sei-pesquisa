 ...
 * 2014-11-12
 * Vers„o do Gerador de CÛdigo: 1.0
 *
 * Classe de Busca na soluÁ„o de indexaÁ„o solr.
 *
 */
...
class MdPesqBuscaProtocoloExterno{
    ...
    public static function executar($parametrosSolr)
    {
      ...
      // FILTRA POR TIPO DE PROCESSO
        ...
        for ($i = 0; $i < count($registros); $i++) {
            ...
            $idProcesso     = InfraSolrUtil::obterTag($registros[$i], 'id_proc', 'long');
            ...
            if(!empty($objProtocoloDTO)){
                ...
                if($objProtocoloDTO->getStrStaProtocolo() == ProtocoloRN::$TP_PROCEDIMENTO){
                    ...
                    if($isPublico){
                        ...
                        $objProcessoDTO->setDblIdProtocolo($idProcesso);
            ...
            // Protege contra a n√£o idexa√ß√£o no solr quando o processo ou documento passa de p√∫blico para restrito ou quando o documento possui intima√ß√µes n√£o cumpridas:
            if(!empty($objProtocoloDTO)){
            ...
            if ($objProtocoloDTO) {
                if ($objProtocoloDTO->getStrStaProtocolo() != ProtocoloRN::$TP_PROCEDIMENTO) {
                    ...
                    // Esconde highlight se intima√ß√£o do documento ou anexos n√£o estiver cumprida:
                    ...
                    // INCLUIDO 21/12/2015 Substitui data de gera√ß√£o para data de assinatura de documentos gerados
            ...
            $titulo = $strNomeTipoProcedimento . " n∫" . $tituloLinkNumeroProcesso;
            $strProtocoloDocumento = "";
            if (empty($dados["protocolo_documento_formatado"]) == false) {
                if ($objDocumentoDTO == null) {

                    // TODO esse trecho remove registros que tem protocolo mas n„o tem o documento
            ...
            // REMOVE TAGS DO T√çTULO
            $tituloCompleto = preg_replace("/&lt;.*?&gt;/", "", $tituloCompleto);
            if ($objProtocoloDTO) {
                if(!$isPublico && !$bolLinkMetadadosProcessoRestrito) {

                    $titulo = $strNomeTipoProcedimento . " n¬∫ " . $dados["protocolo_processo_formatado"];
                    if($objProtocoloDTO->getStrStaProtocolo() != ProtocoloRN::$TP_PROCEDIMENTO){
                        $titulo .= " (".trim($dados["identificacao_protocolo"]).")";
                    }
                    $strProtocoloDocumento = $dados["protocolo_documento_formatado"];
                    ...
                    if ($objHipoteseLegalDTO != null) {
                        $snippet = '<b>HipÛtese Legal de RestriÁ„o de Acesso: ' . $objHipoteseLegalDTO->getStrNome() . ' (' . $objHipoteseLegalDTO->getStrBaseLegal() . ')</b>';
            ...
            if ($objProtocoloDTO) {
                ...
                $html .= "<td colspan=\"2\" class=\"pesquisaTituloEsquerda\" data-prot=\"".$objProtocoloDTO->getStrProtocoloFormatado()."\">";
                $html .= $tituloCompleto;
                $html .= "</td>\n";
                ...
                if (is_array($arrMetatags) && count($arrMetatags) > 0) {
                    ...
                    foreach ($arrMetatags as $nomeMetaTag => $valorMetaTag) {
                        if($nomeMetaTag != 'Usu·rio' && $valorMetaTag != ''){
    ...
    private static function retornoVazio($termo)
    {
        ...
        $semResultados .= "<div class=\"sem-resultado\">Sua pesquisa pelo termo <b>" .$termo. "</b> n„o encontrou nenhum protocolo correspondente. <br/><br/>Sugestıes:";
        $semResultados .= "<ul>";
        ...
        $semResultados .= "<li>Tente palavras-chave mais gen√©ricas.</li>";
        $semResultados .= "</ul>";
        $semResultados .= "</div>";
        $semResultados .= "</consultavazia>";
